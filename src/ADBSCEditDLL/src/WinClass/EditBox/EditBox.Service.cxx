    ...
    bool EditBox::serviceAutoToolTip(std::string const & s, int32_t pos, bool isargs)
    {
        do
        {
            ...
            if (!isargs)
            {
                if (!f_descriptionAutoHighlight(m_txtdata.lightText, r->second))
                    break;
            }
            else
            {
                if (!f_baseAutoHighlight(m_txtdata.lightText, r->second))
                    break;
            }

            if (m_txtdata.lightText.count)
                cmd(
                    SCI_CALLTIPSETHLT,
                    m_txtdata.lightText.base,
                    (m_txtdata.lightText.base + m_txtdata.lightText.range[m_txtdata.lightText.curent++] - 1)
                );
        }
        while (0);
        return true;
    ...
    void EditBox::serviceAutoToolTipShiftArgs()
    {
        do
        {
            ...
            uint32_t n, off = m_txtdata.lightText.base;
            for (n = 0U; ((n < m_txtdata.lightText.count) && (n < m_txtdata.lightText.curent)); n++)
                off += m_txtdata.lightText.range[n];

            cmd(
                SCI_CALLTIPSETHLT,
                off,
                (off + m_txtdata.lightText.range[m_txtdata.lightText.curent++] - 1)
            );
        }
        while (0);
    }
